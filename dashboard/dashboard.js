...
function loadQuizzes(uid) {
    ...
    onSnapshot(q, (querySnapshot) => {
        ...
        querySnapshot.forEach((doc) => {
            const quiz = doc.data(); const quizId = doc.id; const quizUrl = `${window.location.origin}/quiz/index.html?id=${quizId}`;
...
document.getElementById('save-quiz-btn').addEventListener('click', async () => {
    ...
    try {
        if (currentEditQuizId) {
            ...
            const quizUrl = `${window.location.origin}/quiz/index.html?id=${docRef.id}`;
            
            // --- UPDATED: Success message and form reset for NEW quizzes ---
            Swal.fire({
                title: 'Quiz Saved!',
                icon: 'success',
                html: `
                ...
                // This runs AFTER the user closes the popup
